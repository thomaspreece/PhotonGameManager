...
Type DatabaseSearchPanelType Extends wxPanel
	...
	Function HyperLinkFun(event:wxEvent)
		Local DatabaseSearchPanel:DatabaseSearchPanelType = DatabaseSearchPanelType(event.parent)
		Local MessageBox:wxMessageDialog = New wxMessageDialog.Create(Null, "Goto website: " + DatabaseSearchPanel.SourceText.GetURL() + "?~n(Note that Photon GameManager does not take any responsibility for the content or quality of the link and the site it points to)" , "Question", wxYES_NO | wxNO_DEFAULT | wxICON_QUESTION)
	...
	Method SourceChanged()
		...
		'Get Lua File
		Local LuaFile:String = LUAFOLDER + "Search" + FolderSlash + SearchSource.GetValue() + ".lua"
	...
	Method Search()
		...
		Local LuaFile:String = LUAFOLDER + "Search" + FolderSlash + Self.SearchSource.GetValue() + ".lua"
		If LuaHelper_LoadString(LuaVM, "", LuaFile) <> 0 then
			LuaMutexUnlock()
			Return
		EndIf
