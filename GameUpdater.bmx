...
FolderCheck()
LogName = "Log-GameUpdater "+CurrentDate()+" "+Replace(CurrentTime(),":","-")+".txt"
CreateFile(LOGFOLDER+LogName)

AppTitle = "Photon-GameUpdater"
...
Local ReadGames = ReadDir(GAMEDATAFOLDER)
Local Game:String
Local GameNode:GameType = New GameType
Local Platform:PlatformType

Print "This will update your game library to be compatable with the latest version of Photon GameManager"
Print "Please backup your game library before continuing in case there are any problems"
Local ContinuePrompt:String = Input("Type 'yes' to continue: ")
If ContinuePrompt="yes" Or ContinuePrompt="YES" Or ContinuePrompt="Yes" Then 
	
ElseIf ContinuePrompt="backup" Or ContinuePrompt="BACKUP" Or ContinuePrompt="Backup" 
	End 
Else
...
EndIf
Repeat
	Game = NextFile(ReadGames)
	If Game = "." Or Game = ".." Then Continue
	If Game = "" Then Exit
	Print("Updating: "+Game)
	GameNode.GetGame(Game)
	Platform = GlobalPlatforms.GetPlatformByName(GameNode.Plat)
	If Platform = Null Then
		Print("Null Platform")
	Else
		Print("Platform updated to number: "+Platform.ID)
		GameNode.PlatformNum = Platform.ID
	EndIf
	Print("Saving as: "+GameNode.Name+"---"+GameNode.PlatformNum)
	GameNode.SaveGame()
	Print ""
Forever
